...
- **`Enum/`** — plain backed enums (no methods); marked `@codeCoverageIgnore` since they have zero executable statements. Includes `Enum/Http/Method` (string) and `Enum/Http/StatusCode` (int), used internally by `JiraClient` instead of magic strings/status codes — kept as native PHP enums rather than pulling in `symfony/http-foundation`'s `Request::METHOD_*`/`Response::HTTP_*` constants, since that package is currently a dev-only dependency (via `symfony/framework-bundle`), not a runtime one.
...
- **`Serializer/`** — `SerializerFactory::create()` builds the Symfony `Serializer` used by every repository by default (unless one is injected), including three custom normalizers:
  ...
  - `CommentNormalizer`, `WorklogNormalizer` and `FieldsNormalizer` all also handle Atlassian Document Format (ADF) rich-text nodes (`Comment::$body`, `Worklog::$comment`, `Fields::$description`) via the `Xen3r0\Adf\Node\Block\Document` class from the `xen3r0/adf-converter` dependency, since ADF documents need `Document::load()`/`jsonSerialize()` rather than generic object (de)normalization.
...
Several models that are both read from and written to the API (`Comment`, `Version`, `CustomFieldOption`, `Fields`, `Component`, `Link`, `Worklog`) declare a `public const WRITE_GROUP` and tag the writable properties with `#[Groups(groups: [self::WRITE_GROUP])]`. Repository methods that POST/PUT pass `['groups' => X::WRITE_GROUP]` as serialization context so only the intended subset of properties is sent, instead of the full deserialized shape. Follow this pattern for any new model that needs both directions.
